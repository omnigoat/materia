...
namespace atma
{
	...
	auto buf_housekeeping(void* buf) -> housekeeping_t*
	{
...
namespace atma
{
	constexpr uint32 header_size = 4;
	constexpr uint32 header_state_bitsize = 2;
	constexpr uint32 header_state_bitshift = 30;
	constexpr uint32 header_type_bitsize = 2;
	constexpr uint32 header_type_bitshift = 28;
	constexpr uint32 header_alignment_bitsize = 2;
	constexpr uint32 header_alignment_bitshift = 26;
	constexpr uint32 header_size_bitsize = 26;
	constexpr uint32 jump_command_body_size = sizeof(void*) + sizeof(uint32);

	constexpr uint32 header_state_bitmask = aml::pow2(header_state_bitsize) - 1;
	constexpr uint32 header_type_bitmask = aml::pow2(header_type_bitsize) - 1;
	constexpr uint32 header_alignment_bitmask = aml::pow2(header_alignment_bitsize) - 1;
	constexpr uint32 header_size_bitmask = aml::pow2(header_size_bitsize) - 1;
...
inline auto base_lockfree_queue_t::available_space(uint32 wp, uint32 ep, uint32 bufsize, bool contiguous) -> uint32
	...
	// headerer_t
	inline auto headerer_t::size() const -> uint32
	{
	...
	inline auto headerer_t::alignment() const -> uint32
	{
		return 4 * aml::pow2(alignment_);
	}

	inline auto headerer_t::data() const -> void*
	{
		return buf_ + (aml::alignby(op_ + header_size, alignment()) % buffer_size());
	}

	// allocation_t
	inline allocation_t::allocation_t(byte* buf, uint32 wp, allocstate_t state, alloctype_t type, uint32 alignment, uint32 size)
	...
	inline auto allocation_t::encode_byte(byte b) -> void*
	{
	...
	inline auto allocation_t::encode_uint16(uint16 i) -> void
	{
		encode_byte(byte(i & 0xff));
		encode_byte(byte(i >> 8));
	}

	inline auto allocation_t::encode_uint32(uint32 i) -> void
	{
		encode_uint16(i & 0xffff);
		encode_uint16(i >> 16);
	}

	inline auto allocation_t::encode_uint64(uint64 i) -> void
	{
	...
	template <typename T>
	inline auto allocation_t::encode_pointer(T* p) -> void
	{
...
#  error unknown pointer size??
	...
	inline auto allocation_t::encode_data(void const* data, uint32 size) -> void
	{
	...
	template <typename T>
	inline auto allocation_t::encode_struct(T&& x) -> bool
	{
		// require contiugous
	...
	inline auto allocation_t::encode_data(unique_memory_t const& data) -> void
	{
		static_assert(sizeof(uint64) <= sizeof(uintptr), "pointer is greater than 64 bits??");
	...
	// decoder_t
	inline decoder_t::decoder_t()
		: headerer_t(nullptr, 0, 0, 0)
	{}

	inline decoder_t::decoder_t(byte* buf, uint32 rp)
	...
	inline decoder_t::decoder_t(decoder_t&& rhs)
	...
	inline auto decoder_t::decode_byte(byte& b) -> void
	{
		b = buf_[p_];
		p_ = (p_ + 1) % buffer_size();
	}
	
	inline auto decoder_t::decode_uint16(uint16& i) -> void
	{
		byte* bs = (byte*)&i;
	...
	inline auto decoder_t::decode_uint32(uint32& i) -> void
	{
		byte* bs = (byte*)&i;
	...
	inline auto decoder_t::decode_uint64(uint64& i) -> void
	{
		byte* bs = (byte*)&i;
	...
	template <typename T>
	inline auto decoder_t::decode_pointer(T*& p) -> void
	{
		decode_struct(p);
	}

	inline auto decoder_t::decode_data() -> unique_memory_t
	{
		uint32 size;
		decode_uint32(size);
	...
	template <typename T>
	inline auto decoder_t::decode_struct(T& x) -> void
	{
