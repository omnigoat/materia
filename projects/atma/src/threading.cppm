...
#define ATMA_SCOPED_LOCK_II(counter, args) \
	if (::std::scoped_lock<BOOST_PP_SEQ_FOR_EACH_I(ATMA_SCOPED_LOCKM, _, args)> BOOST_PP_CAT(scoped_lock_, counter) {BOOST_PP_SEQ_ENUM(args)}; true)
...
// -------------
//
export namespace atma::this_thread
{
	auto set_debug_name(char const* thread_name)
	{
#if ATMA_PLATFORM_WINDOWS
		auto os_thread_name = platform_interop::make_platform_string(thread_name);
		SetThreadDescription(GetCurrentThread(), os_thread_name.data());
#endif
	}
}
...
export namespace atma
{
#if 1 // no args = no problems?
	void enqueue_function_to_queue(lockfree_queue_t& queue, atma::function<void()> const& f)
	{
	...
	void enqueue_function_to_queue(lockfree_queue_t& queue, atma::function<void(Args...)> const& f)
	{
...
{
	template <typename... Args>
	void consume_queue_of_functions(lockfree_queue_t& queue, Args&&... args)
	{
...
export namespace atma
{
	...
	auto thread_work_provider_t::enqueue_against(work_token_t& tk, function_t const& f) -> void
	{
	...
	auto thread_work_provider_t::enqueue_against(work_token_t& tk, function_t&& f) -> void
	{
	...
	auto thread_work_provider_t::enqueue_repeat_against(work_token_t& tk, repeat_function_t const& f) -> void
	{
	...
	auto thread_work_provider_t::enqueue_repeat_against(work_token_t& tk, repeat_function_t&& f) -> void
	{
		// this lambda mutates itself every time it is called to use a different token-index
...
export namespace atma
{
	...
	inplace_engine_t::inplace_engine_t()
		: running_{false}
		, queue_{}
	{}

	inplace_engine_t::inplace_engine_t(defer_start_t, uint32 bufsize)
		: running_{false}
		, queue_{bufsize}
	{}

	inplace_engine_t::inplace_engine_t(uint32 bufsize)
		: running_{true}
		, queue_{bufsize}
	{
#if 0
	...
	inplace_engine_t::inplace_engine_t(void* buf, uint32 bufsize)
		: running_{true}
		, queue_{buf, bufsize}
	{
#if 0
	...
	auto inplace_engine_t::is_running() const -> bool
	{
		return running_;
	}

#if 1
	auto inplace_engine_t::reenter(std::atomic<bool> const& good) -> void
	{
		while (good)
		{
...
#if 0
	auto inplace_engine_t::ensure_running() -> void
	{
		if (!is_running())
		{
			running_ = true;
	...
	auto inplace_engine_t::signal(function_t const& fn) -> void
	{
		if (!running_)
			return;
	...
	auto inplace_engine_t::signal(function_t&& fn) -> void
	{
		if (!running_)
			return;
	...
	auto inplace_engine_t::signal_evergreen(repeat_function_t const& fn) -> void
	{
		auto sg = [&, fn]() {
			if (!fn())
				return;
	...
	auto inplace_engine_t::signal_block() -> void
	{
		if (!running_)
			return;
...
export namespace atma
{
	...
	thread_pool_t::thread_pool_t(uint threads)
		: queue_{ threads * 256 }
	{
	...
	auto thread_pool_t::thread_count() const -> size_t
	{
		return threads_.size();
	}

	auto thread_pool_t::enqueue(function_t const& fn) -> void
	{
	...
	auto thread_pool_t::enqueue(function_t&& fn) -> void
	{
		queue_.with_allocation(sizeof(internal_function_t) + (uint32)fn.external_buffer_size(), 4, true, [&fn](auto& A) {
	...
	auto thread_pool_t::enqueue_repeat(repeat_function_t const& fn) -> void
	{
		function_t rfn = [&, fn]{
			if (!fn())
				return;
	...
	auto thread_pool_t::enqueue_repeat(repeat_function_t&& fn) -> void
	{
	...
	auto thread_pool_t::worker_thread_runloop(thread_pool_t* pool, std::atomic_bool& running) -> void
	{
		char buf[128];
